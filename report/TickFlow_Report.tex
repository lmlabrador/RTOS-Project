...
\begin{itemize}
  \item \textbf{LO1}: Design and analyze real-time task models with explicit timing constraints.
  \item \textbf{LO2}: Apply fixed-priority preemptive scheduling and evaluate schedulability.
  \item \textbf{LO3}: Implement and validate RTOS scheduling algorithms in software.
  \item \textbf{LO4}: Analyze the impact of mixed-criticality task interactions on system safety.
  \item \textbf{LO5}: Use formal timing analysis tools (RTA) to assess scheduling correctness.
\end{itemize}
...
\subsection{Schedulability Validation}
...
With a 35\% overrun probability per HI-task job, the expected number of mode switches per hyperperiod can be estimated. In 1200ms, T1 releases $\lfloor 1200/100 \rfloor = 12$ jobs and T3 releases $\lfloor 1200/150 \rfloor = 8$ jobs. Expected overruns: $(12+8) \times 0.35 = 7$ per hyperperiod, each triggering a potential mode switch (subject to already being in HI mode). Simulation results confirm this order of magnitude.
...
\subsection{Scheduling Analysis Validation}
The optimizer evaluates 8 configurations ($4 \times 2$) and scores each:
\begin{equation}
...
\begin{enumerate}
  ...
  \item \textbf{WCET Estimation Matters}: The 35\%/65\% overrun probability model revealed that even a moderate overrun rate causes frequent mode switches, dramatically impacting LO-task throughput---a practical lesson in system margin selection.
...
\textbf{LO1 (Task modeling)}: Implementing the \texttt{TaskModel} interface and configuring multi-parameter tasks deepened understanding of the periodic task abstraction and its limitations in modeling aperiodic workloads.

\textbf{LO2 (Scheduling analysis)}: Computing RTA by hand for the default task set, then verifying against the simulator's output, solidified the fixed-point iteration concept and its practical convergence behavior.

\textbf{LO3 (Algorithm implementation)}: Writing the scheduler step function required careful attention to job release timing, preemption semantics, and deadline detection---translating textbook algorithms into working code.

\textbf{LO4 (Mixed-criticality)}: Observing mode switches and LO-task suspensions in the live simulator made the trade-offs of mixed-criticality scheduling tangible: accepting LO-task failures as a safety mechanism is a deliberate, principled design decision.

\textbf{LO5 (Formal analysis)}: The optimization engine's evaluation of 8 scheduling configurations demonstrated that formal analysis can guide priority assignment beyond intuition alone.
